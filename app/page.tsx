...
export default function Home() {
  ...
  useEffect(() => {
    // Check if user is already session-active to skip landing
  ...
  const handleAuth = async (intent?: 'errand' | 'runner') => {
    const redirectTo = `${window.location.origin}/auth/callback${intent ? `?intent=${intent}` : ''}`;
  ...
  if (loading) return <div className="bg-black min-h-screen" />; // Prevent flash
      ...
      <section className="relative h-[90vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6 max-w-4xl">
          ...
          <p className="mt-6 text-lg md:text-xl text-gray-400 max-w-xl mx-auto leading-relaxed">
            ...
            <span className="text-white">Deploy a runner to the streets in seconds.</span>
          </p>
